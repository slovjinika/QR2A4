...
START_NUM = 121
END_NUM = 300  # 6 blocks * 30 stickers = 180 stickers max per page
...
BLOCKS_X = 2
BLOCKS_Y = 3
STICKERS_PER_BLOCK = 30  # Изменено на 30 стикеров (сетка 6х5)
...
for idx, text in enumerate(data_list):
    ...
    # Расчет сетки 6х5 внутри блока (6 колонок, 5 строк)
    inside_sticker_idx = sticker_idx % STICKERS_PER_BLOCK
    sx = inside_sticker_idx % 6   # Столбец: 0..5
    sy = inside_sticker_idx // 6  # Строка: 0..4
    
    # Вычисляем общую ширину и высоту группы из 30 стикеров
    group_w_mm = (ROTATED_STICKER_W_MM * 6) + (GAP_X_MM * 5)
    group_h_mm = (ROTATED_STICKER_H_MM * 5) + (GAP_Y_MM * 4)
    
    # Идеальное центрирование сетки 6х5 внутри блока 105х99 мм
    ...
    # Калибровочный сдвиг строк блоков
    if by == 0:
...
if pages:
    ...
    print(f"Done! PDF generated. 30 stickers (6x5) per block. Size: 14x14mm.")
